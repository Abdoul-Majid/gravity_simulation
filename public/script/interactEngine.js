...
export default class InteractEngine {
  ...
  mouseMove(event) { 
    ...
    if (this.rightMousePressed) { // same for the right mouse button
      this.engine.moveRightMouse(mouseX,mouseY);
    }
  }
  ...
  mouseUp(event) {
    ...
    if(!this.leftMousePressed){
      this.engine.abandonObstacle();
    }
    if(!this.rightMousePressed){
      this.engine.rightMouseStopped();
    }
  }
  ...
  keyDown(event) {
    switch(event.key) {
      ...
      case 'i': case 'I': this.engine.increaseFactor();break;
      case 'd': case 'D': this.engine.decreaseFactor();break;
      case 'f': case 'F': this.engine.applyFrictionForce();break;
      case 'o': case 'O': this.engine.disableFrictionForce();break;
      default:break;
    }
  }
